...
func RegisterAdmin(r *gin.Engine, cfg config.Config, deps *app.Deps) {
	...
	api := r.Group("/api")
	{
		...
		protected := api.Group("", adminHandler.RequireAdmin())
		{
			...
			protected.GET("/avatars/:id/videos", avatarHandler.ListVideos)
			protected.POST("/avatars/:id/videos", avatarHandler.CreateVideo)
			protected.DELETE("/videos/:id", avatarHandler.DeleteVideo)
			// Knowledge base: avatar -> collection (知识库) -> documents (文档)
			protected.POST("/avatars/:id/knowledge-collections", knowledgeHandler.CreateCollection)
